    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (userID.equals(loginUser.getUserID())) {
                request.setAttribute("ERROR_MESSAGE", "User dang dang nhap, khong the xoa!!!");
            } else {
                UserDAO dao = new UserDAO();
                boolean check = dao.deactivateUser(userID);
                if (check) {
                    url = SUCCESS;
                }
            }
        } catch (Exception e) {
            log("Error at DeactivateController: " + e.toString());
        } finally {
